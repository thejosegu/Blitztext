...
public class FeedbackTab : UserControl
{
    ...
    private UIElement BuildUi()
    {
        ...
        stack.Children.Add(MakeCardWith("Ereignis-Log", inner =>
        {
            ...
            var clearBtn = new Button { Content = "Log leeren", Margin = new Thickness(0, 0, 6, 0) };
            clearBtn.Click += (_, _) => { AppLog.Clear(); Refresh(); };
            var copyBtn = new Button { Content = "Log kopieren" };
            copyBtn.Click += (_, _) =>
            {
                var lines = AppLog.GetAll();
                if (lines.Count > 0)
                    System.Windows.Clipboard.SetText(string.Join("\n", lines));
            };
            var openBtn = new Button { Content = "Log-Ordner öffnen", Margin = new Thickness(6, 0, 0, 0) };
            openBtn.Click += (_, _) =>
            {
                var dir = System.IO.Path.GetDirectoryName(AppLog.LogFilePath)!;
