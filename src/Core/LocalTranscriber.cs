...
public static class LocalTranscriber
{
    ...
    public static bool IsWarmingUp    { get; private set; }
    public static string? LastLoadError { get; private set; }

    /// <summary>Fired on the calling thread after WarmUp succeeds or fails.</summary>
    public static event Action<bool, string?>? LoadStatusChanged;

    /// <summary>
    /// Loads and caches the model immediately (call at startup to avoid first-use delay).
    /// </summary>
    public static void WarmUp(string modelPath, string language = "auto")
    {
        IsWarmingUp = true;
        LoadStatusChanged?.Invoke(false, null);   // notify UI: "wird geladen…"
        try
        {
            LastLoadError = null;
            EnsureProcessor(modelPath, language);
            IsWarmingUp = false;
            LoadStatusChanged?.Invoke(true, null);
        }
        catch (Exception ex)
        {
            IsWarmingUp   = false;
            LastLoadError = ex.Message;
            LoadStatusChanged?.Invoke(false, ex.Message);
            throw;
        }
    }
        ...
        string language = "auto")
    {
        ...
        EnsureProcessor(modelPath, language);
        if (!wasLoaded)
            LoadStatusChanged?.Invoke(true, null);   // lazy load completed
