...
public sealed class HotkeyListener : IDisposable
{
    ...
    private int     _debounceGen;   // incremented on every new or cancelled debounce
    ...
    private void OnKeyDown(uint vk)
    {
        ...
        lock (_lock)
        {
            ...
            if (modeToStop == null)
            {
                ...
                _debounceTimer = new Timer(_ => TryTrigger(gen), null, 50, Timeout.Infinite);
            }
        }

        if (modeToStop != null)
        {
            try { _onStop(modeToStop); }
            catch (Exception ex) { AppLog.Add($"HotkeyListener.OnKeyDown Fehler: {ex.Message}"); }
        }
    }
    ...
    {
        string? modeToStop = null;
        string? modeToStart = null;

        lock (_lock)
        {
            if (_activeMode == null && _getRecordMode() == "toggle" && _debounceTimer != null)
            {
                int bestLen = 0;
                foreach (var (mode, spec) in _getHotkeys())
                {
                    var combo = ParseHotkey(spec);
                    if (combo.Count > 0 && combo.SetEquals(_pressed) && combo.Count > bestLen)
                    {
                        bestLen = combo.Count;
                        modeToStart = mode;
                    }
            ...
            _debounceTimer?.Change(Timeout.Infinite, Timeout.Infinite);
            _debounceTimer = null;
            _debounceGen++;   // invalidate any pending TryTrigger
        ...
        {
            try { _onStop(modeToStop); }
            catch (Exception ex) { AppLog.Add($"HotkeyListener.OnKeyUp Fehler: {ex.Message}"); }
        }

        if (modeToStart != null)
            StartMode(modeToStart, rollbackActiveMode: true, errorPrefix: "HotkeyListener.OnKeyUp Fehler");
    }

    private void TryTrigger(int gen)
    {
        string? bestMode = null;
        bool reserveActiveMode = false;

        lock (_lock)
        {
            _debounceTimer = null;
            // If OnKeyUp fired and incremented _debounceGen, this trigger is stale
            ...
            if (bestMode != null)
            {
                ...
                reserveActiveMode = true;
            }

        }

        if (bestMode != null)
            StartMode(bestMode, rollbackActiveMode: reserveActiveMode, errorPrefix: "HotkeyListener.TryTrigger Fehler");
    }

    private void StartMode(string mode, bool rollbackActiveMode, string errorPrefix)
    {
        try
        {
            if (!_onStart(mode) && rollbackActiveMode)
            {
                lock (_lock)
                {
                    if (_activeMode == mode)
        ...
        catch (Exception ex)
        {
            ...
            AppLog.Add($"{errorPrefix}: {ex.Message}");
        }
    }
